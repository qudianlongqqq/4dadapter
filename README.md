...
### Setup on Linux RTX 50-series GPUs
RTX 50-series GPUs use Blackwell CUDA capability, so the old CUDA 12.1 conda stack
in `env.yml` is not the right target. Use the CUDA 12.8 wheel-based environment:

```bash
conda env create -f env-5090.yml
conda activate etflow-5090
python3 -m pip install -e .
```

Make sure the server NVIDIA driver is new enough for CUDA 12.8 before creating the
environment.
