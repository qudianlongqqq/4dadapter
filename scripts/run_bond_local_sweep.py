"""Run the ETFlow bond-local velocity regularization sweep sequentially."""
...
class SweepExperiment:
    name: str
    bond_velocity_loss_weight: float
    bond_velocity_on_rotatable_only: bool


EXPERIMENTS: Tuple[SweepExperiment, ...] = (
    SweepExperiment("all_lam0", 0.0, False),
    SweepExperiment("all_lam001", 0.001, False),
    SweepExperiment("all_lam003", 0.003, False),
    SweepExperiment("all_lam01", 0.01, False),
    SweepExperiment("rot_lam0", 0.0, True),
    SweepExperiment("rot_lam001", 0.001, True),
    SweepExperiment("rot_lam003", 0.003, True),
    SweepExperiment("rot_lam01", 0.01, True),
)

METRIC_COLUMNS: Tuple[str, ...] = (
    "train/loss",
    "train/flow_matching_loss",
    "train/bond_local/loss",
    "train/bond_local/q_pred_abs_mean",
    "train/bond_local/q_target_abs_mean",
    "train/bond_local/valid_bonds",
    "train/bond_local/parallel_loss",
    "train/bond_local/perp_loss",
    "val/loss",
    "val/flow_matching_loss",
    "val/bond_local/loss",
    "val/bond_local/q_pred_abs_mean",
    "val/bond_local/q_target_abs_mean",
    "val/bond_local/valid_bonds",
    ...
    record.update(
        {
            ...
            "bond_velocity_loss_weight": str(
                experiment.bond_velocity_loss_weight
            ),
            "bond_velocity_on_rotatable_only": str(
                experiment.bond_velocity_on_rotatable_only
            ),
...
def _build_command(
    ...
    command = [
        sys.executable,
        str(project_root / "scripts" / "train_bond_local.py"),
        "--config",
        str(config_path),
        "--output_dir",
        str(output_dir),
        ...
        str(args.batch_size),
        "--bond_velocity_loss_weight",
        str(experiment.bond_velocity_loss_weight),
    ...
    if experiment.bond_velocity_on_rotatable_only:
        command.append("--bond_velocity_on_rotatable_only")
    return command
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the ETFlow bond-local regularization sweep sequentially."
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        "--base_output_dir",
        default="logs_sweep/bond_local_5000steps",
    )
...
def main() -> int:
    ...
    for experiment in EXPERIMENTS:
        ...
        print(f"bond_velocity_loss_weight: {experiment.bond_velocity_loss_weight}")
        print(
            "bond_velocity_on_rotatable_only: "
            f"{experiment.bond_velocity_on_rotatable_only}"
        )
