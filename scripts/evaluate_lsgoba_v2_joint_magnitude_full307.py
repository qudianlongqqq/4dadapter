...
def generate() -> None:
    ...
    try:
        for pos,sample_id in enumerate(ids,start=1):
            srcrow=by_sample[sample_id]; item=by_item[srcrow["molecule_id"]]; graph=item["graph"].to(device); source=srcrow["source"].to(device=device,dtype=torch.float64); record=torch.load(source_paths[sample_id],map_location="cpu",weights_only=False)
            ...
            with torch.enable_grad():
                p1=distribution_parameters(graph,model=v1,variant="B"); d1=direction(source,graph,p1)
                pred=v2.geometry(graph); p2={**pred,"bond_sigma":graph.bond_fixed[:,1],"angle_sigma":graph.angle_fixed[:,1]}; d2=direction(source,graph,p2)
                _,state,embedding,_=batch_source_directions_and_state(source,[graph],p2,pred["node_embedding"]); tau_learned=float(v2.magnitude(embedding,v2.normalized_state(state)).detach())
            candidates={"Raw":(source,{"accepted":True,"fallback":False},{"final_rms":0.0,"final_atom_max":0.0,"atom_scale_min":1.0},0.0)}
            for method,d,tau in (("Original-v1-003",d1,.003),("V2-network-fixed003",d2,.003),("V2-network-fixed005",d2,.005),("V2-learned",d2,tau_learned)):
                output,safe,trust=deploy(source,graph,d,tau); candidates[method]=(output,safe,trust,tau)
            for method,(output,safe,trust,tau) in candidates.items():
                delta=output-source; rms=float(delta.square().sum(-1).mean().sqrt()); atom=float(torch.linalg.vector_norm(delta,dim=-1).max()) if delta.numel() else 0.0
            ...
            if pos%100==0: print(json.dumps({"stage":"DEV_TEST_COORDINATES","records":pos,"expected":5000}),flush=True)
    finally:
...
def summarize() -> None:
    ...
    zero=float((tau<.001).mean())>.9; ceiling=float((tau>=.009).mean())>.9; orig=table.loc["Original-v1-003"]; v2=table.loc["V2-learned"]; pb_ok=v2.PB>=orig.PB-cfg["validation"]["posebusters_nonregression_tolerance"]; rollback_ok=v2.rollback_rate<=orig.rollback_rate+cfg["validation"]["rollback_absolute_increase_max"]; pareto=((v2.V3D>orig.V3D and v2.mean_graph_rms<=orig.mean_graph_rms) or (v2.V3D>=orig.V3D and v2.mean_graph_rms<orig.mean_graph_rms)) and pb_ok and rollback_ok and not zero and not ceiling; decision="PROMISING" if pareto else "NO_GO"
    payload={"schema_version":"lsgoba-v2-full307-result-v1","V2_STATUS":"COMPLETE","summary":table.reset_index().to_dict("records"),"tau":{"mean":float(tau.mean()),"median":float(np.median(tau)),**quant,"lt001_fraction":float((tau<.001).mean()),"le003_fraction":float((tau<=.003).mean()),"between003005_fraction":float(((tau>.003)&(tau<=.005)).mean()),"gt005_fraction":float((tau>.005).mean()),"ge009_fraction":float((tau>=.009).mean())},"tau_collapse_zero":zero,"tau_collapse_ceiling":ceiling,"pb_nonregression":bool(pb_ok),"rollback_gate":bool(rollback_ok),"v2_pareto_improvement":bool(pareto),"decision":decision,"extra_training_exposure_confound":True,"formal_causal_claim_allowed":False,"xtb_stage":"NOT_STARTED","orca_stage":"NOT_STARTED","formal_test_started":False,"formal_test_records_read":0,"frozen_holdout_records_read":0}; atomic_json(REPORT/"V2_FULL307_RESULT.json",payload)
    atomic_text(REPORT/"09_V2_FULL307_RESULT.md",f"# LSGO-BA v2 Full307 result\n\nDecision: **{decision}** (development feasibility only; extra-training exposure confound remains).\n\n```csv\n{table.reset_index().to_csv(index=False).strip()}\n```\n\nLearned tau mean/median/P10/P25/P75/P90/P95: {tau.mean():.7f} / {np.median(tau):.7f} / {quant['p10']:.7f} / {quant['p25']:.7f} / {quant['p75']:.7f} / {quant['p90']:.7f} / {quant['p95']:.7f} Å. Collapse-zero={zero}; collapse-ceiling={ceiling}; PB non-regression={pb_ok}; rollback gate={rollback_ok}; Pareto improvement={pareto}. No xTB, ORCA, docking, formal test, or frozen holdout was accessed.")
